...
export class ClientSideImplementation {
    ...
    async createSigningKeys(): Promise<CryptoKeyPair> {
        const kp = await crypto.subtle.generateKey(SIGNING_ALGORITHM_KEY_PARAMS, true, ["sign", "verify"]) as CryptoKeyPair
        return kp
    }
    ...
    initWrapKey(resolver: Resolver): WrapKey {
        return async (toWrap: CryptoKey, publicKeyUrl: string) => {
            ...
            if (wrapKeys.length == 0) {
                ...
                const wrapWith = await crypto.subtle.importKey("jwk", wrapKeys[0], WRAPPING_ALGORITHM_KEY_PARAMS, true, ["wrapKey"])
